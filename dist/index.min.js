...
const contracts={token:contractToken};var initContract=async function(a,b){invariant(b,'gethClient is not defined'),invariant(a,'set current contract name');const c=truffleContract(contracts[a]);return c.setProvider(b.provider),c};

var _slicedToArray$1=function(){function a(a,b){var c=[],d=!0,e=!1,f=void 0;try{for(var g,h=a[Symbol.iterator]();!(d=(g=h.next()).done)&&(c.push(g.value),!(b&&c.length===b));d=!0);}catch(a){e=!0,f=a;}finally{try{!d&&h['return']&&h['return']();}finally{if(e)throw f}}return c}return function(b,c){if(Array.isArray(b))return b;if(Symbol.iterator in Object(b))return a(b,c);throw new TypeError('Invalid attempt to destructure non-iterable instance')}}();var checkToken=async function(a,b){invariant(a,'address is not defined'),invariant(a.startsWith('0x'),'address should starts with 0x'),invariant(b,'gethClient is not defined');try{if('0x'!==(await b.method('getCode')(a))){const e=await initContract('token',b),f=await e.at(a);var c=await Promise.all([f.name(),f.symbol(),f.decimals()]),d=_slicedToArray$1(c,3);const g=d[0],h=d[1],i=d[2];return{name:g,symbol:h,decimals:i,address:a,contract:f}}return!1}catch(a){return!1}};

var add0x=function(a){return'string'!=typeof a||a.startsWith('0x')?a:'0x'+a};

const getBalance=get('c[0]'); const GAS_LIMIT_DEFAULT=2e5; const GAS_PRICE_MAX=new bignumber(1e11);class Account{constructor({gethClient:a,address0x:b,limitGasPrice:c=GAS_PRICE_MAX,throwGasPriceError:d=!1}){invariant(a,'gethClient is not defined'),invariant(b,'address is not defined'),invariant(b.startsWith('0x'),'address should starts with 0x'),this.throwGasPriceError=d,this.limitGasPrice=new bignumber(c),this.geth=a,this.address=b,this.tokens={},this.nonce=0;}async addToken(a){const b=await checkToken(a,this.geth);return!!b&&void(this.tokens[a]=b)}async getCurrencyBalances(){let a=[this.getBalance()];for(const b in this.tokens)a.push(this.getTokenBalance(b));const b=await Promise.all(a),c={};for(let a in b){const d=0===parseInt(a)?'0x':Object.keys(this.tokens)[a-1];c[d]=b[a].toString();}return c}async getCurrencies(){let a=[{address:'0x',symbol:'eth',name:'Ethereum'}];for(const b in this.tokens){const c=this.tokens[b];a.push({symbol:c.symbol.toLowerCase(),address:c.address,name:c.name,decimals:c.decimals});}return a}async getBalance(){const a=await this.geth.method('getBalance')(this.getAddress());return a?a+'':a}async getTokenBalance(a){a||(a=Object.keys(this.tokens)[0]);const b=await this.tokens[a].contract.balanceOf(this.address);return getBalance(b)}getAddress(){return this.address}getGasLimit(){return GAS_LIMIT_DEFAULT}async getGasPrice(){let a=await this.geth.method('getGasPrice')();if(a.gt(this.limitGasPrice)){if(this.throwGasPriceError)throw new Error('Too much gas price '+a.toFormat());a=GAS_PRICE_MAX;}return a}async sendTokens(a,b,c,d,e){c||(c=Object.keys(this.tokens)[0]),this.nonce||(this.nonce=await this.geth.method('getTransactionCount')(this.getAddress()));const f=toHex(b);d=d||toHex((await this.getGasLimit())),e=e||toHex((await this.getGasPrice()));const g=this.tokens[c].contract.transfer(this.normalizeTarget(a),f,{from:this.getAddress(),gasLimit:d,gasPrice:e,nonce:toHex(this.nonce)});return this.nonce++,new TransactionResult(g,this.geth)}async sendEther(a,b,c,d){this.nonce||(this.nonce=await this.geth.method('getTransactionCount')(this.getAddress())),c=c||toHex((await this.getGasLimit())),d=d||toHex((await this.getGasPrice()));const e=toHex(b),f={from:this.getAddress(),gasLimit:c,gasPrice:d,value:e,to:this.normalizeTarget(a),nonce:toHex(this.nonce)};return this.nonce++,this.geth.sendTransaction(f)}normalizeTarget(a){return a instanceof Account?a.address:add0x(a)}}var Account_1=Account;
...
const web3$1=new web3;var formatEther={fromWei:web3$1.fromWei,toWei:web3$1.toWei};
...
const fromWei=formatEther.fromWei; const toWei=formatEther.toWei; const environment=config.environment||'development'; const createGethClient=memoization_1.memoize(function(a){return new GethClient_1(a)}); const createProvider=memoization_1.memoize(providerFactory);function createSonmFactory(a,b={}){const c=createProvider(a),d=createGethClient(c),e={gethClient:d,config:config[environment]};return Object.assign(e,b),{gethClient:d,createAccount:async function(a){const b=add0x(a);e.address0x=b;const c=new Account_1(e);return await c.addToken(config[environment].contractAddress.token),c},createTxResult:function(a){return new TransactionResult(a,d)},setPrivateKey:function(a){const b=add0x(a);c.setPrivateKey(b);}}}var blockchainApiJs={createSonmFactory,utils:{recoverPrivateKey,add0x,fromWei,toWei}};
